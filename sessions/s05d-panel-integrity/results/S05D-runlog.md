...
Generated 2026-08-19T04:27:19+00:00 (UTC).
...
| Load panels and presence masks | 0.1 s |
| Phase 1 clock mapping and 20-session comparison | 0.2 s |
| Phase 2 zero-variance windows at source (reads the 107 MB S04 Globex parquet) | 3.2 s |
| Phase 3 padding and fill | 0.0 s |
| Phase 4 daily aggregation exposure | 1.0 s |
| Phase 5 reports | ~2 min |

Compute total 4.4 s; session total well under the 15-minute expectation. No bottleneck.

## Inputs read (all read-only)

- `sessions/s05-reliability-mcs/results/panel_ES_GLOBEX_B0.npz`, `panel_ES_RTH_B0.npz`
- `sessions/s04-repairs-diagnostics/results/bars_GLOBEX.parquet`, `s04_build.json`
- `sessions/s05b-defect-and-estimator-audit/results/cache/present_ES_{GLOBEX,RTH}.npz` (presence masks regenerated in S05B by re-running S03 `build_panels`)
- `sessions/s03-data-noise/src/analysis.py` (source quotation only)

## Constants

- Globex column offset 1080 minutes (18:00 NY), modulo 1440, columns kept where slot < 1380.
- RTH column offset 570 minutes (09:30 NY), 390 columns.
- Clock hours examined: 13:00, 14:00, 15:00 NY = Globex columns 1140, 1200, 1260.
- Session sample: 20 sessions at fixed stride `len(dates)//20` across the full span, no randomness.
- Zero-variance window sample: 50 drawn with `DataFrame.sample(random_state=0)`, the only stochastic step in the session.
- Padded sub-bar definition: contains no return whose two endpoint minutes are both present.
- M grid (Globex 1day): 5, 6, 10, 12, 23, 46, 138, 345, 1379.
- Free-intercept fit: `scipy.optimize.curve_fit` on Var(log RV_M) = c + A M^b, start [min(y), 1.0, -0.5].
