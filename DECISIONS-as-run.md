...
Every post-hoc deviation from a frozen spec is recorded here with date and reasoning.
...
1. Interleaved-subsample estimator REMOVED from the spec. Screening simulation
   ...
   to the failure. Non-overlapping temporal splits retained as a separate arm.
...
4. DEPENDENCY STRUCTURE CHANGED. The reliability parameter lambda is now sourced
   from the realized-quarticity route of Bollerslev, Patton and Quaedvlieg (2016),
   which has published asymptotic theory. The alternative estimators are demoted to
...
6. Session 1 pre-registration parameters rewritten after audit. The prior draft
...
7. S01 verdict on D6/D7 is CONDITIONAL. Noise-to-signal ratios {0.001, 0.01} were
   asserted by the analyst, not measured on ES/NQ. S02 therefore reports breakdown
...
8. S01 omitted noise-robust and jump-robust volatility proxies entirely. Every
   ...
   S02 adds them as the primary proxy arm.
9. S01 DGPs hold integrated variance constant within each window (to be verified in
   code). E2's pass on D1-D4 may therefore be an artifact, since contiguous halves
   estimate the same quantity only under within-window constancy. S02 adds
   within-window volatility variation and a diurnal profile.
10. E1 reduced from 4 arms x 4 lag sets to 2 arms x 2 lag sets for compute reasons,
    retaining a_exp (baseline) and d_model (best performer on D3, 0.956 at L1-10),
    at L1-5 and L1-10. This is a compute decision citing S01 evidence, recorded here
...
11. SCOPE.md is not present anywhere in the working tree (checked at session
    ...
    same way in the data. All 68 are excluded and enumerated; the 18-vs-68
...
12. NSR denominators: E[IV] in NSR = omega^2/E[IV] is the signature-plot
    ...
    degeneracy, and the S03 data confirm: N2 exceeds N1 by roughly an order of
...
14. E4 does not use RQ. It uses (2/M)*RQ/RV^2, which is scale-free and equals 2/M
    ...
    ratio. The quantity that decides E4 is therefore unmeasured. S05 measures it.
15. PASS CRITERION CHANGED. No band on reliability recovery is used, because no
    ...
    an invented band is not defensible. The criterion becomes whether Model Confidence
...
17. S03, S04 and S05 pinned no environment and, so far as the code shows, logged no
    seeds. S05A retrofits what is retrofittable and determines by measurement whether
    S05 requires a rerun, rather than presuming one.
18. Seed recovery is NOT attempted for the MCS bootstrap. Instead, composition
    stability across many independent seeds is measured. Composition stable across
    seeds is a stronger claim than bitwise reproducibility of one draw.
19. The S03/S04 pipeline consistency test runs here. Two copies of rules 1-4 and 6
    exist (S03 pipeline.main and the S04 re-execution) and nothing has asserted they
    agree. A failure invalidates S04 and S05 downstream.
20. Phase 2 comparison span and Phase 4 verification cells are fixed by deterministic
    rule stated in the session prompt before execution, since neither could be named
    in advance of reading S05. Phase 3 execution order is fixed for the same reason.
...
21. S05 section 5 reports M4_HARQ QLIKE of order 1e296 with R-squared near -630 and
    ...
    cells. S05B locates and counts the responsible forecasts. No filter is applied here.
22. S05 Part E returns definite MCS compositions for cells whose section 5 loss column
    is inf or nan for every model. S05B inspects the array the MCS actually consumes
...
23. S05 section 5 applies one lam_hat per cell without recording which of the six
    Part C estimators produced it. S05B determines provenance by source inspection.
24. Every Part E composition and every section 5 metric is treated as unverified until
    items 21 to 23 are resolved.
...
26. Var(log IV) is a property of the IV series at a given horizon and cannot depend on
    ...
    is therefore an assumption-free validity test, and is adopted as the empirical
...
27. The prior conclusion that microstructure noise is not a live explanation is
    ...
    signature-plot rise is an M-dependent RV distortion and is tested directly here.
...
29. The M grid is extended below the S05 range to M of 5, 6 and 10 where session length
    ...
    regime is deferred to a later session.
30. Horizons 1day, 1h and 30min are all in scope for RTH. Globex 1h and 30min are
    excluded for the whole session, since their repair belongs to a later session and
    conditional mid-session scope expansion would not be pre-registered. RTH intraday
    carries the lowest measured lambda in the study and is not dropped for convenience.
31. Boundary treatments B0 and B1 both run on the full grid. Measured on the S05 output,
...
32. The IC-IR column reported in S05 section 5 has no recorded definition. S05B
    establishes it by source inspection. The analytic ICIR attenuation result under
    heterogeneous lambda is pre-registered as an intended derivation for a later
    session and is not computed here.
...
33. S05B Phase 3 places 82.4 percent of Globex zero-variance windows at or after
    ...
    Every Globex result in S03, S04, S05 and S05B is provisional until this resolves.
...
37. No positive control has been run on the aggregation or reliability path. S05E runs
    one before any estimator is selected or any repair is specified. An estimator
    selection rule is NOT adopted until S05E returns.
...
39. INVARIANT TESTS. Five assertions are written before any repair and run inside the
...
40. FORECAST FILTER. The Bollerslev, Patton and Quaedvlieg insanity filter is applied
    ...
    and per model. The filter is applied identically in every cell regardless of whether
    it fires.
41. RGARCH IS NOT FILTERED. At ES/GLOBEX/B1/30min M5_RGARCH returns 69,119 non-positive
    ...
    S06R diagnoses whether the estimated parameters violate stationarity, and where they
    do, RGARCH is reported as failing to estimate in that cell and the model set is
...
43. RANGE ESTIMATORS ARE REBUILT FROM TRUE BAR HIGH AND LOW. The current M6_PARK and
    M6_GK take max and min of the forward-filled cumulative close path, which is not a
...
44. S01's E3 ERROR-CORRELATION GATE IS RE-RUN on the corrected Parkinson and
    Garman-Klass series. The 0.805 correlation that triggered exclusion was computed on
    misconstructed inputs. The outcome is reported, not presumed.
...
46. ADMISSIBLE ESTIMATOR PAIR. E1_a and E1_d are dropped. E1 fails on rough volatility
    by S01's own record and E1_d violates the [0,1] bound at three or four of nine grid
    points in every 1day cell. E2 and E4 are retained and BOTH are reported throughout,
    ...
    0.128 under jumps. Neither is designated primary. No further synthetic arms are run
...
47. MULTIPLICITY. The Part E family size of 96 comparisons is stated explicitly against
    the effective sample. No familywise correction is applied and its absence is
...
49. LEVEL SANITY CHECK. The fitted intercept implies Var(log IV) near 1.02, so sd(log
    ...
    whether its level was plausible. S06R reports the level against the sample's own
    volatility range.
50. Holdout boundary 2024-01-01 is unchanged and untouched.
...
53. RGARCH DIAGNOSIS was run in S06R on 16 cells that exclude every GLOBEX intraday
    ...
    forecasts. The question is unanswered and is answered here, on those 8 cells.
54. MULTIPLICITY REPORTING. The S06R pre-specified cell ES/RTH/B0/30min stands as
    reported and is NOT replaced. Largest effective sample selects the cell where the
    ...
    logged before its comparison is computed.
55. SPY REPLICATION. The proxy-error exponent is refitted on SPY 1-second data,
    ARCX.PILLAR and XNAS.ITCH treated as two independent venue-level replications and
    never pooled. Purpose is to establish whether b near -0.44 is instrument-specific
...
56. SPY CAVEATS FIXED IN ADVANCE. Results are labelled two-venue and cover roughly 33
    ...
    fill-versus-volatility correlation it requires is reported here as a by-product.
...
53. RGARCH DIAGNOSIS in S06R ran on 16 cells excluding every GLOBEX intraday cell,
    ...
    question is unanswered and is answered here on those 8.
54. MULTIPLICITY. The S06R pre-specified cell ES/RTH/B0/30min stands and is NOT
    ...
    logged before its comparison is computed.
55. SPY DATA is at `/Users/GualyCr/Downloads/DataBento Data/SPY 1s Data`, outside the
    repository. The session inventories it before reading, records a SHA-256 manifest
    into the repo, and rebuilds from raw. The derived parquets under `data/` are NOT
    consumed, because SCOPE section 3 records them carrying the uncorrected early-close
    defect in their `_daily_partial` rollup.
56. SPY FILL IS A LIVE CONFOUND, not a caveat. Median fill of 0.667 to 0.686 means
    ...
    ignorable. The exponent is therefore fitted under BOTH calendar-time forward-filled
...
57. SPY RESULTS ARE TWO-VENUE, covering roughly 33 percent of consolidated volume,
    ...
    conditions on nothing, and its three required measurements are reported here.
...
61. THE MCS LEG IS EVALUATED AS K2. On the clean RTH geometry in S07, composition
    ...
    unavailable in all eight intraday cells and where the filter fired hardest. K2 is
    determined here on repaired losses and reported whichever way it falls. The null
...
64. THE HOLDOUT IS NOT OPENED IN S08. An earlier draft of this session proposed
...
67. K2 IS NOT REPORTABLE WITHOUT A PLACEBO. S-B and S-C condition on different variables
    ...
    conditioning on a second F_{t-1} variable, no proxy involvement, establishes the
...
72. HOLDOUT OPENS ONCE, in Phase 6, for ES and NQ from 2024-01-01 to 2026-08-14. No
...
73. NO FURTHER MEASUREMENT SESSION FOLLOWS S09. Roughness, vol-of-vol from the fitted
    ...
    reliability are recorded as further work and not pursued.
...
76. ENVIRONMENT EVENT, 2026-08-19. The project virtual environment failed on pandas
    ...
    intercept fit against stored values. Phase 1 of S09-PRE records whether the installed
    versions matched the lock before the rebuild; a mismatch means the environment
...
79. THE PROJECT ENVIRONMENT LIVES OUTSIDE THE iCLOUD SYNC SCOPE, at
    /Users/GualyCr/venvs/obs-space-vol. The failure of 2026-08-19 is most consistent
...
80. ITEM 73 IS AMENDED. It barred further measurement sessions. S10 runs because the
    headline exponent has never carried an uncertainty estimate: S08 Phase 4
    bootstrapped c and reported c_lo and c_hi, and reported nothing for b. Every claim
    of the form "materially flatter than sampling theory" rests on a point estimate
    with no error bar. Post hoc, disclosed. S10 adds no new data and opens no holdout.
...
82. POOLING ACCOUNTS FOR PART OF THE GAP AND WAS NEVER FOLLOWED UP. S05E Phase 3 found
    ...
    is a pooling artifact by the project's own measurement. No within-year exponent has
...
83. THE RETURN-DISTRIBUTION HYPOTHESIS WAS NEVER TESTED. trigamma(M/2) assumes Gaussian
    ...
    would catch this does not exist.
84. THE WITHIN-WINDOW ROUGHNESS HYPOTHESIS WAS NEVER TESTED EITHER. S05E's A4 arm
    ...
    variance can live. A4's negative result is empty and is withdrawn. Roughness is
    tested here only if Phase 5 leaves the gap open.
85. RS-UP AT ES/RTH/1h IS UNDER SUSPICION. It moved from in-sample R-squared 0.0128 to
    ...
    with an alignment defect than a regime change. Diagnosed in sample; the holdout is
    not reopened.
...
87. THE S09 PHASE 5 PARTITION IS VOID and is recomputed here. The defect reaches 32 of
...
88. HOLDOUT RE-EVALUATION, DISCLOSED IN FULL. The holdout was opened once in S09 under
    defective predictor construction. S11 re-evaluates ONLY the candidates the defect
...
89. sigma_w IS CALIBRATED, NOT CHOSEN. S10 Phase 6 flagged sigma_w as its weakest
    ...
    against trigamma. S11 calibrates sigma_w per cell to reproduce the measured ratio
    and re-tests the S10 roughness rejection against calibrated variation.
90. THE EXPONENT AS A PROXY SPECIFICATION TEST. Var(log X_M) = c + A*M^b is fitted for
...
91. FINANCIAL APPLICATIONS, ALL PRE-REGISTERED BEFORE ANY RESULT IS SEEN. The project's
    ...
    of the second kind are tested: risk-limit breaches, inverse-MSE combination weights,
...
95. THE S11 PHASE 10 CONVEXITY MAGNITUDE IS VOID. It applied the Brockhaus-Long
    ...
    DETERMINATION is expected to survive since a hand recompute gives roughly 19 percent
...
96. THE S11 PHASE 6 MECHANISM CLAIM IS PROVISIONAL PENDING CALIBRATION VERIFICATION.
    ...
    RQ/RV^2 is far flatter in sigma_w than expected. Verified here by reporting implied
...
100. THE WITHIN-WINDOW DISPERSION MECHANISM IS PARTIAL, NOT GENERAL. S12 narrowed
     ...
     geometry. Something else drives the RTH anomaly and S13 tests one candidate.
...
104. THE ANOMALY IS LARGELY HISTORICAL AND ITS SHAPE IS UNTESTED. At -0.047 per year
     ...
     0.357. S13 tests whether the closing is a smooth decline or a level shift at a
     date, since a break at 2020 implies a different account from a continuous trend.
105. THE HOLDOUT IS READ ONCE MORE, in Phase 2 only, for the K7 application. No
...
106. THE FIRST FOUR APPLICATIONS WERE CHOSEN BADLY, disclosed. K3, K4, K5 and K7 all
     ...
     contamination. S14 selects on that criterion.
...
109. K10, HURST BIAS. The standard estimator regresses the q-th absolute moment of
     ...
     0.02 in every cell.
110. THE HOLDOUT IS READ ONCE MORE, in Phase 1 only, for K8. Fifth opening. The
...
113. THE TREND HAS NOT BEEN TESTED AGAINST THE CONDITIONING CONTROL. S14 Phase 5 shows
     ...
     trend of -0.047 per year has never been regressed jointly against that control. If
...
115. ITEM 114 IS AMENDED, disclosed. It barred further measurement sessions. S16 runs
     ...
     the observable reduces either. Post hoc, disclosed, and the rule is fixed here
...
117. THE SOURCE ASSUMES AWAY WHAT THIS PROJECT MEASURES. That paper states its 5-minute
     ...
     than noise. The moving average is an unmotivated noise treatment. S16 replaces it
...
121. HOLDOUT OPENS A SIXTH TIME, in Phase 3 only. The running count is stated in the
...
127. THE OBSERVATION-NOISE VARIANCE IS DERIVED, NOT TUNED. Var(eps) = (1 - lambda) *
     ...
     66. No value is chosen to improve any outcome, and a sensitivity across scalings
     of 0.25, 0.50, 0.75 and 1.00 is reported so the result's dependence on the
     magnitude is visible.
128. HOLDOUT OPENS A SEVENTH TIME, in Phase 4 only. Running count stated in the paper.
...
129. NO MEASUREMENT IS PERFORMED IN S18. Every figure in the paper is read from a
     ...
     session report, or filled from context. Restating figures from memory has produced
...
131. SECTION 5 IS NOT DRAFTED IN S18. It carries K1 through K12 and its inclusion
     decisions are authorial. S18 drafts sections 4, 6, 7 and 8 against the approved
     outline and leaves section 5 as a stub with its table generated.
132. THE LATEX BUILD IS VERIFIED BEFORE PROSE IS FINALISED. A separate project is
     already blocked on a pandoc output-quality problem, so S18 builds a minimal
     document to PDF first and halts if the build fails, rather than discovering it at
     the end.
...
134. THREE FIGURES FOR ONE QUANTITY. The count of in-sample states moved by the emission
     ...
     disagree. Both are re-read from artifacts here. Restating figures from memory has
...
137. SECTION 3 SHIPPED EMPTY TWICE. S18 and S19 both produced a document with a Section 3
     ...
     all reference its result. S20 writes it from the text supplied in this prompt and
     verifies its inclusion in the built PDF before the session closes.
138. SECTION 2 CONTAINS FOUR DROPPED-TEXT DEFECTS. "spreads are removed at the symbol
     ...
     without a clause. They are repaired here from the artifacts that supply the figures.
139. THE PAPER SAYS TWELVE KILL CONDITIONS AND TABLE 1 LISTS THIRTEEN. The renumbering
     ...
     introduction are corrected, and Table 1 regains the margin column dropped in the
